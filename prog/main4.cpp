...
int main(int, char **argv) {
    ...
    while (infile >> data.S >> data.K >> data.r >> data.v >> data.T) {
        ...
        std::cout << "Underlying:      " << data.S << std::endl;
        std::cout << "Strike:          " << data.K << std::endl;
        std::cout << "Risk-Free Rate:  " << data.r << std::endl;
        std::cout << "Volatility:      " << data.v << std::endl;
        std::cout << "Maturity:        " << data.T << std::endl;

        std::cout << "Dividents:        ";
        for (int i = 0; i < data.D.payments.size(); ++i) {
            std::cout << data.D.payments[i] << " ";
        }
        std::cout << std::endl;

        if ((call > 0) && (call_w_div > 0)) {
            std::cout << "Call Price:                     " << call << std::endl;
            std::cout << "Call Price with dividents:      " << call_w_div << std::endl;
        }
        if ((put > 0) && (put_w_div > 0)) {
            std::cout << "Put Price:                      " << put << std::endl;
            std::cout << "Put Price with dividents:       " << put_w_div << std::endl;
        }
